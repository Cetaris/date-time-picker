...
export class OwlMonthViewComponent<T> implements OnInit, AfterContentInit, OnDestroy {
  ...
  selectDate(date: number): void {
    ...
    if (date) {
      ...
      selected = this.dateTimeAdapter.addCalendarDays(this.firstDateOfMonth, daysDiff);

      this.selectedChange.emit(selected);
      this.userSelection.emit();
    } else {
      // Set to today's date
      selected = this.dateTimeAdapter.addCalendarDays(this.firstDateOfMonth, new Date().getDate() - 1);
